...
sub submit
{
    ...
    my $job_project = $description->project();
    unless(defined($job_project))
    {
        $self->log("  Project not specified, using default value $DEFAULT_PROJECT");
        $job_project = $DEFAULT_PROJECT;
    }
    $self->log("  Job assigned to " . $job_project);
    $rc = $sge_job_script->print("#\$ -P ". $job_project ."\n");
    if (!$rc)
    {
        return $self->respond_with_failure_extension(
                "print: $sge_job_script_name: $!",
    ...
    {
      #####
      # It's a multiple job and we use job arrays
      #
    ...
    foreach my $tuple ($description->environment())
    {
        ...
        elsif ($tuple->[0] eq "PE_MODE" && $tuple->[1] =~ /single/i)
        {
            $mpi_pe = "mpisingle";
        }
        else
    ...
    $rc = $sge_job_script->print("# Change to directory requested by user: $directory\n" .
                           "#$ -cwd \n" .
                           "cd $directory \n");
    if (!$rc)
    {
    ...
    if($mpi_pe && $mpi_pe ne "NONE" && $mpi_pe ne 'no') {
        ...
        # let's reserve resources
        $rc = $sge_job_script->print("#\$ -R y\n");
        if (!$rc)
        {
            return $self->respond_with_failure_extension(
                    "print: $sge_job_script_name: $!",
    ...
    {
        #####
        # It's MPI job or single job count>1
        #
        ...
        if ( $description->jobtype() eq "mpi" ) 
        {
            ...
            else
            {
                ...
                # Using non-Sun's MPI.
                #
                $rc = $sge_job_script->print("$mpirun -ssh -np ". $description->count()
        ...
        {
            $self->log(" user will fork the job himself: " . $description->executable() . " args:" . $args);
    ...
    elsif( ( $description->jobtype() eq "multiple" ) && ( $description->count() > 1 ) )
    {
        #####
        # (multiple & count>1) -> Using job arrays
        #
